...
CODE_SECTION(VLIB_matchTemplate_mmt, ".text:optimized")
int32_t VLIB_matchTemplate_mmt(uint8_t img[restrict],
                           int32_t imgWidth,
                           int32_t imgHeight,
                           int32_t imgPitch,
                           ...
                           uint8_t scratch[restrict])
{
    ...
    int16_t    yJumpoutScoreWidth = outScoreWidth * yDirJump;
    int32_t    yJumpTempImgHeight;
    int32_t     i, j, l, k;
    ...
    if((tempImgWidth < 4) || (tempImgHeight < 4) || (img == NULL) ||
       ...
       (outScorePitch < outScoreWidth) || (tempImgHeight * tempImgWidth > 64*48)) {
        ...
        /*Sliding Horizontal sum of original image pixels. Size of the horzontal
          ...
          jianghao:对于Y方向jump为2的情况，每隔一行计算一次sum，减少一半时间
         */
        if ((yDirJump == 2) && (tempImgHeight % 2 == 1))
        {
            yJumpTempImgHeight = tempImgHeight + 1;
            rcpTempArea = _rcpsp(tempImgWidth * (tempImgHeight / yDirJump + 1));
        }
        else
        {
            yJumpTempImgHeight = tempImgHeight;
            rcpTempArea = _rcpsp(tempImgWidth * (tempImgHeight / yDirJump));
        }
        
        rcp4      = _rcpsp(4.0f);

        for( l = 0; l < imgHeight; l += yDirJump ) {
        ...
        for( l = 0; l < outputWidthAlign4; l += 4 ) {
            ...
            /*First four element of each row*/
            for( i = 0; i < tempImgHeight; i += yDirJump) {
                ...
                pu16TempPtr   += yJumpoutScoreWidth;
            }
            ...
            *pu16CurWinAvg  = (uint16_t)f32temp1;
            ...
            pu16CurWinAvg += (yJumpoutScoreWidth - 4);

            pu16TempPtr1 = &pu16CurWinHSumOrig[l + (0 * outScoreWidth)];
            pu16TempPtr2 = &pu16CurWinHSumOrig[l + (yJumpTempImgHeight * outScoreWidth)];

            /*4th element to last four byte aligned location*/
            for( k = yDirJump; k < outScoreHeight; k += yDirJump ) {
                ...
                *pu16CurWinAvg  = (uint16_t)f32temp1;
                ...
                pu16TempPtr1   += yJumpoutScoreWidth;
                pu16TempPtr2   += yJumpoutScoreWidth;
                pu16CurWinAvg  += (yJumpoutScoreWidth - 4);
            }
        }
        ...
        /* Two seperate loop has been implemented to avoid shift operations in core loop.
           ...
           jianghao:48*48 is larger than 2048, but no risk
        */
        if((tempImgWidth == 48) && (((uint32_t)tempImg & 0x7U) == 0x0) &&
            ((tempImgPitch & 0x3U) == 0x0) && (((uint32_t)img & 0x3U) == NULL) &&
            ((imgPitch & 0x3U) == NULL)) {
            //Vps_printf("start width 48 template match!\n");
        ...
        } else if((tempImgWidth == 32) && (((uint32_t)tempImg & 0x7U) == 0x0) &&
                    ((tempImgPitch & 0x3U) == 0x0) && (((uint32_t)img & 0x3U) == NULL) &&
                    ((imgPitch & 0x3U) == NULL)) {
            //Vps_printf("start width 32 template match!\n");
            for( l = 0; l < outScoreHeight; l+=yDirJump ) {
                for( k = 0; k < outScoreWidth; k+=xDirJump ) {
                    normCurWinSq   = 0;
                    normCurTempMul = 0;
        ...
        } else if((tempImgWidth == 64) && (((uint32_t)tempImg & 0x7U) == 0x0) &&
                    ((tempImgPitch & 0x3U) == 0x0) && (((uint32_t)img & 0x3U) == NULL) &&
                    ((imgPitch & 0x3U) == NULL)) {
            //Vps_printf("start width 64 template match!\n");
            for( l = 0; l < outScoreHeight; l+=yDirJump ) {
                for( k = 0; k < outScoreWidth; k+=xDirJump ) {
                    normCurWinSq   = 0;
                    normCurTempMul = 0;
            ...
            for( l = 0; l < outScoreHeight; l+=yDirJump ) {
                for( k = 0; k < outScoreWidth; k+=xDirJump ) {
                    ...
                    for( i = 0; i < tempImgHeight; i++ ) {
                        ...
                        for( j = 0; j < (tempImgWidth & 0xFFFFFFFCU); j += 4 ) {
                            /*Current pixels*/
                            u32temp1  = _mem4_const(pu8temp1);
                            pu8temp1 += 4;
                            /*Template pixels*/
                            u64temp1  = _mem8_const(ps16temp1);
                            ps16temp1+= 4;
                            /*Current pixels, changed to Q2*/
                            u64temp2  = _dshl2(_unpkbu4(u32temp1), (uint32_t)0x2U);
                            /*Mean subtracted current pixels*/
                        ...
                        for( j = tempImgWidth & 0xFFFFFFFCU; j < tempImgWidth; j++ ) {
                            /*Q2 format*/
                            normCurWinSq += (((img[((l + i) * imgPitch) + (j + k)] * 4) - curWinAvg) * ((img[((l + i) * imgPitch) + (j + k)] * 4) - curWinAvg));
                            /*Q2 format*/
                            normCurTempMul += (((img[((l + i) * imgPitch) + (j + k)] * 4) - curWinAvg) * tempImg[(i * tempImgPitch) + j]);
                        }
                    }
...
//获取数组中最大值的位置和值
void getMaxLoc(float* array, int width, int height, int jump, int* max_x, int * max_y, float* max_value)
{
    int i,j;
    int x=0,y=0;
...
{
    temp_buffer = (uint8_t*)Utils_memAlloc(UTILS_HEAPID_L2_LOCAL, MAX_TEMPL_LONG_EDGE * MAX_TEMPL_SHORT_EDGE, 32);
    image_buffer = (uint8_t*)Utils_memAlloc(UTILS_HEAPID_L2_LOCAL, MAX_SEARCH_IMG_SIZE, 32);
    templ_q_buffer = (int16_t*)Utils_memAlloc(UTILS_HEAPID_L2_LOCAL, MAX_TEMPL_LONG_EDGE*MAX_TEMPL_SHORT_EDGE*sizeof(int16_t), 32);
...
void match_template_classic_free_buffer()
{
    ...
    status = Utils_memFree(
                ...
                temp_buffer,
                MAX_TEMPL_LONG_EDGE*MAX_TEMPL_SHORT_EDGE);
    ...
    UTILS_assert(SYSTEM_LINK_STATUS_SOK == status);
        ...
        status = Utils_memFree(
                UTILS_HEAPID_L2_LOCAL,
                templ_q_buffer,
                MAX_TEMPL_LONG_EDGE*MAX_TEMPL_SHORT_EDGE*sizeof(int16_t));
...
int32_t match_template_classic_dsp(MtInParam* param, MtOutResult* out_buf)
{
  ...
  //检测框使用int型，和X86保持一致
  Rect rect_int;
  Rect rect_search_int;

  for (i = 0; i < param->pad_num; i++)
  {
    //根据尺寸计算模板的仿射变换矩阵  
    //缩放比例
    rect_int.left = param->templ_pad[i].left;
    rect_int.right = param->templ_pad[i].right;
    rect_int.top = param->templ_pad[i].top;
    rect_int.bottom = param->templ_pad[i].bottom;
  
    if ((param->match_mode[i] & 0xF) == 2)
    {
        templ_w = 64;
        templ_h = 48;
    }
    else if ((param->match_mode[i] & 0xF) == 1)
    {
        templ_w = 48;
        templ_h = 48;
    }
    ...
    scale_x = (rect_int.right - rect_int.left)/(float)templ_w;
    if ((rect_int.bottom - rect_int.top)/scale_x > templ_h)
    {
        scale_y = (rect_int.bottom - rect_int.top)/(float)templ_h;
    }
    else
    {
        scale_y = scale_x;
    ...
    Vps_printf("MTC resize w:%d, h:%d, jump:%d!\n", templ_w, templ_h, x_y_jump);
    ...
    if (templ_h * templ_w > MAX_TEMPL_LONG_EDGE * MAX_TEMPL_SHORT_EDGE)
    {
      //对于这种情况，直接用原始的框的位置，不进行模板匹配
      out_buf->result[i].new_rect.left = rect_int.left;
      out_buf->result[i].new_rect.top = rect_int.top;
      out_buf->result[i].new_rect.right = rect_int.right;
      out_buf->result[i].new_rect.bottom = rect_int.bottom;      
      out_buf->result[i].value = 0.0;
    ...
    rect_search_int.left = param->cur_pad[i].left;
    rect_search_int.right = param->cur_pad[i].right;
    rect_search_int.top = param->cur_pad[i].top;
    rect_search_int.bottom = param->cur_pad[i].bottom;
    ...
    //为优化考虑，将搜索图的宽度扩展到4的整数倍
    img_w = ((img_w+SEARCH_IMG_WIDTH_ALIGN-1) /SEARCH_IMG_WIDTH_ALIGN)*SEARCH_IMG_WIDTH_ALIGN;
    ...
    if ((img_w*img_h > MAX_SEARCH_IMG_SIZE) || (img_h < templ_h) || (img_w < templ_w))
    {
      //对于这种情况，直接用原始的框的位置，不进行模板匹配
...
#if 1
    //模板匹配进行搜索
    ret = VLIB_matchTemplate_mmt(image_buffer,
                                    img_w,
                                    img_h,
                                    img_w,
                                    templ_q_buffer,
    ...
    //找出最大值和所在的点
    getMaxLoc(score_buffer, score_w, score_h, x_y_jump, &max_x, &max_y,&out_buf->result[i].value);
#else
    ...
    // Vps_printf("num:%d!max x-y is:%d,%d!max rect is:%d,%d,%d,%d!max value is:%f!\n",i,max_x,max_y,
