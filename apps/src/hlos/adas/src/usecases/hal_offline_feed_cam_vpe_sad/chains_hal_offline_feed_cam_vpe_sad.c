...
#include "chains_hal_offline_feed_cam_vpe_sad.h"
...
//#define SYNC_VPE_DELTA_IN_MSEC      (16)
//#define SYNC_VPE_DROP_THRESHOLD_IN_MSEC (33)
#define SYNC_VPE_DELTA_IN_MSEC      (2000)
#define SYNC_VPE_DROP_THRESHOLD_IN_MSEC (20000)

extern pthread_mutex_t feed_mutex;
extern int recv_line;
Chains_hal_offline_feed_cam_vpe_sadAppObj g_hal_offline_feed_cam_vpe_sad_AppObj;


static Void chains_hal_offline_feed_cam_vpe_sad_SetSyncPrm(chains_hal_offline_feed_cam_vpe_sadObj *pUcObj)
{
    pUcObj->Sync_vpePrm.syncDelta = SYNC_VPE_DELTA_IN_MSEC;
    pUcObj->Sync_vpePrm.syncThreshold = SYNC_VPE_DROP_THRESHOLD_IN_MSEC;
}


Void chains_hal_offline_feed_cam_vpe_sad_SetNullSrcPrms(Chains_hal_offline_feed_cam_vpe_sadAppObj *pObj,
 ...
 *******************************************************************************
*/
static Void chains_hal_offline_feed_cam_vpe_sad_SetVpePrm(
...
void hal_feed_fillSrcBuf(chains_hal_offline_feed_cam_vpe_sadObj *pObj)
{
    ...
    for (bufCnt = 0;bufCnt < getBufInfoPrm.numBuf;bufCnt ++)
    {
		...
		//Timestamp = 1000000 * (long long)tv->tv_sec + (long long)tv->tv_usec;
		//dstAddr = (UInt8*)((UInt32)pVideoFrame->bufAddr[2]);
		//memcpy(dstAddr, &Timestamp, sizeof(UInt64));
		//Vps_printf("[wagtest]%s:%d sec=%ld usec=%ld\n", __func__, __LINE__,
		//			tv->tv_sec, 
		//			tv->tv_usec);
        ...
        OSA_memCacheWb(
                ...
                ((UInt32)pVideoFrame->bufAddr[1] + \
                            (NULLSRC_OUT_WIDTH * (NULLSRC_OUT_HEIGHT/2))));
		/*OSA_memCacheWb(
                (UInt32)pVideoFrame->bufAddr[2],
                ((UInt32)pVideoFrame->bufAddr[2] + 32));*/
 ...
 *******************************************************************************
*/
void chains_hal_offline_feed_cam_vpe_sad_nullsrc_restart(Chains_hal_offline_feed_cam_vpe_sadAppObj *pObj)
{
 ...
 *******************************************************************************
 *
 * \brief   chains_hal_offline_feed_cam_vpe_sad_nullsrc_pause
 *
 ...
 *******************************************************************************
*/
void chains_hal_offline_feed_cam_vpe_sad_nullsrc_pause(Chains_hal_offline_feed_cam_vpe_sadAppObj *pObj)
{
    /* Puase Null source */
    System_linkControl(
            pObj->ucObj.NullSourceLinkID,
 ...
 *******************************************************************************
*/
Void chains_hal_offline_feed_cam_vpe_sad_SetAppPrms(chains_hal_offline_feed_cam_vpe_sadObj *pUcObj,Void *appObj)
{
    Chains_hal_offline_feed_cam_vpe_sadAppObj *pObj
            = (Chains_hal_offline_feed_cam_vpe_sadAppObj*)appObj;
    ...
    chains_hal_offline_feed_cam_vpe_sad_SetNullSrcPrms(pObj, &pUcObj->NullSourcePrm);
    ...
    chains_hal_offline_feed_cam_vpe_sad_SetVpePrm(&pUcObj->VPE_resize_0Prm,
                                1,
    ...
    chains_hal_offline_feed_cam_vpe_sad_SetVpePrm(&pUcObj->VPE_resize_1Prm,
                                1,
    ...
    chains_hal_offline_feed_cam_vpe_sad_SetVpePrm(&pUcObj->VPE_resize_2Prm,
                                1,
    ...
    chains_hal_offline_feed_cam_vpe_sad_SetVpePrm(&pUcObj->VPE_resize_3Prm,
                                1,
    ...
    chains_hal_offline_feed_cam_vpe_sad_SetSyncPrm(pUcObj);
 ...
 *******************************************************************************
*/
void chains_hal_offline_feed_cam_vpe_sad_StartApp(Chains_hal_offline_feed_cam_vpe_sadAppObj *pObj)
{
    //hal_fillSrcBuf(&pObj->ucObj);
    ChainsCommon_statCollectorReset();
    ChainsCommon_memPrintHeapStatus();

    chains_hal_offline_feed_cam_vpe_sad_Start(&pObj->ucObj);
}
 ...
 *******************************************************************************
*/
int chains_hal_offline_feed_cam_vpe_sad_StopApp(Chains_hal_offline_feed_cam_vpe_sadAppObj *pObj)
{
    chains_hal_offline_feed_cam_vpe_sad_Stop(&pObj->ucObj);

    chains_hal_offline_feed_cam_vpe_sad_Delete(&pObj->ucObj);
 ...
 *******************************************************************************
*/
Void chains_hal_offline_feed_cam_vpe_sad(Chains_Ctrl *chainsCfg)
{
    char ch;
    UInt32 done = FALSE;
    Chains_hal_offline_feed_cam_vpe_sadAppObj chainsObj;

    chainsObj.chainsCfg = chainsCfg;

    chains_hal_offline_feed_cam_vpe_sad_Create(&chainsObj.ucObj, &chainsObj);

    chains_hal_offline_feed_cam_vpe_sad_StartApp(&chainsObj);
    ...
    while(!done)
    {
        ...
        switch(ch)
        {
            ...
            case 'P':
                ...
                chains_hal_offline_feed_cam_vpe_sad_printStatistics(&chainsObj.ucObj);
                chains_hal_offline_feed_cam_vpe_sad_printBufferStatistics(&chainsObj.ucObj);
                break;
            default:
    ...
    chains_hal_offline_feed_cam_vpe_sad_StopApp(&chainsObj);
}

static int hal_offline_feed_cam_vpe_sad_get_timestamp(hal_frame_data_t *frmdata, UInt64 m4_timestamp)
{
	struct timeval time;
...
Void hal_offline_feed_cam_vpe_sad_null_link_callback(System_LinkChInfo *pChInfo, Void *pBuf, Void *arg)
{
	static int line = 0;
	int ret = 0;
	...
	memcpy(&frmdata.timestamp, pSysCompBuf->bufAddr[0][0], sizeof(struct timeval));

	//Vps_printf("[wagtest]%s:%d line=%d sec=%ld usec=%ld\n", __func__, __LINE__,
	//		line++,
	//		frmdata.timestamp.tv_sec, 
	//		frmdata.timestamp.tv_usec);
	...
	//Vps_printf("--End-----hal_offline_feed_camera_null_link_callback--------\n");
	return ;
}
